export const groupMessage = {
  messages: [
    {
      text: "Sometimes I'll start a sentence and I don't even know where it's going. I just hope I find it along the way. Like an improv conversation.",
      sender:
        'urn:li:msg_messagingParticipant:urn:li:fsd_profile:ysQsSOJOnvE0-oOaAzG2fKYeiMCALIoPfFBK4kA',
    },
  ],
  participants: [
    'urn:li:msg_messagingParticipant:urn:li:fsd_profile:ysQsSOJOnvE0-oOaAzG2fKYeiMCALIoPfFBK4kA',
    'urn:li:msg_messagingParticipant:urn:li:fsd_profile:GkeAvEzPMsKiaLQOSysOo-CoOBIYKnAfJFA024f',
    'urn:li:msg_messagingParticipant:urn:li:fsd_profile:O4zkACGnoSEFI-L2OMeAKfv0YABPasifQOKoJys',
  ],
  categories: ['PRIMARY_INBOX'],
  createdAt: 1695888754994,
  lastActivityAt: 1695888754994,
  url: 'urn:li:msg_message:(urn:li:fsd_profile:ACoAABBVe30B0tKzB9504vnHyJNPIb9IU-jl7dg,2-MTY5NDc4MDg2MDgwNGIyNDEyOC0wMDQmMjg3NGNlZTgtNzA0Ni00YjMzLWFiNjQtMDliM2JmN2Q4NDM5XzAxMw==)',
}
